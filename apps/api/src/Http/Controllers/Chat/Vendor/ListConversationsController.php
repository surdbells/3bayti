...
/**
 * GET /v3/vendor/chat/conversations?limit=20&offset=0
 *
 * The authenticated vendor's order chat threads across every store they
 * own, newest activity first, with the vendor-side unread total.
 */
final class ListConversationsController
{
    ...
    public function __invoke(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $result = $conversations->findForVendor($vendorIds, $limit, $offset);

        $items = array_map(
            fn (Conversation $c): array => $this->serializer->conversationShape($c, Conversation::PARTY_VENDOR),
            $result['items'],
        );
