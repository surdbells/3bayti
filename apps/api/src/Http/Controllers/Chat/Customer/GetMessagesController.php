...
/**
 * GET /v3/chat/conversations/{uuid}/messages?limit=30&after_id=&before_id=
 *
 * Messages for one of the customer's conversations, oldest→newest, plus the
 * conversation meta so the chat screen opens in a single call. `after_id`
 * fetches newer messages (polling); `before_id` fetches older (scroll-up).
 * Blocked messages are visible only to the party that sent them.
 */
final class GetMessagesController
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        $list = $messages->findForConversation(
            ...
            $beforeId,
            $afterId,
            Conversation::PARTY_CUSTOMER,
        );

        return $this->ok([
            'conversation' => $this->serializer->conversationShape($conversation, Conversation::PARTY_CUSTOMER),
