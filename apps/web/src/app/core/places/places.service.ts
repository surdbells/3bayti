/* =============================================================================
   ...
   Capacitor/SDK/script-loader dependency, so the logic transfers unchanged.
   The only web-specific difference is the environment import path + shape
   (apps/web's flat environment object).
   ...
   Field mask
     ...
     the X-Goog-FieldMask header. We request only "Essentials" tier fields.
...
export class PlacesService {
  ...
  async autocomplete(input: string): Promise<PlaceSuggestion[]> {
    ...
    const body = {
      ...
      /* Restrict to UAE (per current product scope). */
...
/* Generate a v4 UUID without an external dependency. */
